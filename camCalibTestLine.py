...
# Find lines on image
def findLines(sharpen, image):
    edges = cv2.Canny(sharpen, 252, 255, apertureSize=3)
    ...
    lines = cv2.HoughLinesP(
                ...
                minLineLength=120, # Min allowed length of line
...
def findCheckerboardCorners(image):
    ...
    if ret == True:
        ...
        cv2.line(image, (int(xTopL), int(yTopL)), (int(xTopR), int(yTopR)), (0, 255, 0), 2)
        cv2.line(image, (int(xTopL), int(yTopL)), (int(xBotL), int(yBotL)), (0, 255, 0), 2)
        cv2.line(image, (int(xTopR), int(yTopR)), (int(xBotR), int(yBotR)), (0, 255, 0), 2)
        cv2.line(image, (int(xBotL), int(yBotL)), (int(xBotR), int(yBotR)), (0, 255, 0), 2)
        cv2.line(image, (int(xTopL), int(yTopL)), (int(xBotR), int(yBotR)), (0, 255, 0), 2)
        cv2.line(image, (int(xTopR), int(yTopR)), (int(xBotL), int(yBotL)), (0, 255, 0), 2)
        ...
        cv2.putText(image, str(round(dTopLBotR, 2)), (int((xTopL+xBotR)/2), int((yTopL+yBotR)/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)
        cv2.putText(image, str(round(dTopRBotL, 2)), (int((xTopR+xBotL)/2), int((yTopR+yBotL)/2)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 2)

        # Calculate error between opoosite distances
        dTopBot = abs(dTop-dBot)
        dLeftRight = abs(dLeft-dRight)
        dTopLBotRTopRBotL = abs(dTopLBotR-dTopRBotL)
        print("dTopBot: " + str(dTopBot))
        print("dLeftRight: " + str(dLeftRight))
        print("dTopLBotRTopRBotL: " + str(dTopLBotRTopRBotL))
    else:
...
def mainLines():
    ...
    # Distorted image
    sharpen = correctImage(imageDistorted) # Remove noise from image
    findLines(sharpen, imageDistorted) # Na sliki najdemo vse kvadrate in jih označimo
